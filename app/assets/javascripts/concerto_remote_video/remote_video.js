var ConcertoRemoteVideo = {
    ...
    if (video_id.length != 0) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ConcertoRemoteVideo.loadVideoInfo(data);
          ConcertoRemoteVideo.loadVideoPreview(data);
        },
        error: function(e) {
  ...
  loadVideoPreview: function (data) {
  ...
  initHandlers: function () {
    if (!ConcertoRemoteVideo._initialized) {
      $('input#remote_video_config_video_id').on('blur', ConcertoRemoteVideo.getVideoPreview);
      $('select#remote_video_config_video_vendor').on('change', ConcertoRemoteVideo.getVideoPreview);
      $('select#remote_video_config_video_vendor').on('change', ConcertoRemoteVideo.updateTooltip);
      ConcertoRemoteVideo._initialized = true;
    }
  }
};

$(document).ready(ConcertoRemoteVideo.initHandlers);
$(document).on('turbolinks:load', ConcertoRemoteVideo.initHandlers);
